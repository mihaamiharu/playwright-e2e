...
When('I archive the seeded issue via the API', async ({ projectsAPI, sandbox, seededProjectIssue }) => {
  await projectsAPI.archiveItem(sandbox.projectId, seededProjectIssue.projectItemId);
});

When('I unarchive the seeded issue via the API', async ({ projectsAPI, sandbox, seededProjectIssue }) => {
  await projectsAPI.unarchiveItem(sandbox.projectId, seededProjectIssue.projectItemId);
});

Then('the seeded issue should not be visible in any column', async ({ page, seededProjectIssue }) => {
