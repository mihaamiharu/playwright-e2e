...
Then('I should see the table with columns {string}, {string}, and {string}', async ({ page }, col1, col2, col3) => {
...
Then('the seeded issue should appear as a row in the table', async ({ page, seededProjectIssue }) => {
  const row = page.getByRole('row').filter({ hasText: seededProjectIssue.title });
  await expect(row.first()).toBeVisible();
});

Given('seeded table sort test issues exist with prefixes {string} and {string}', async ({ githubAPI, projectsAPI, sandbox, dataManager }, prefixA, prefixZ) => {
  ...
  const issueA = await githubAPI.createIssue(env.github.testRepo, { title: sortTitleA, body: 'Sort test issue' });
  ...
  const issueZ = await githubAPI.createIssue(env.github.testRepo, { title: sortTitleZ, body: 'Sort test issue' });
...
When('I sort the table by the {string} column in descending order', async ({ page }, columnName) => {
  await page.getByRole('button', { name: `${columnName} column options` }).click();
...
Then('the {string} issue should appear before the {string} issue in the table', async ({ page }, firstPrefix, secondPrefix) => {
...
Given('issue {string} exists with status {string} and label {string} in the sandbox project', async ({ githubAPI, projectsAPI, sandbox, dataManager }, issueId, statusName, labelName) => {
  const ts = Date.now();
  ...
  const issue = await githubAPI.createIssue(env.github.testRepo, { title, labels: [labelName], body: `Filter test issue ${issueId}` });
...
Given('issue {string} exists with status {string} and no label in the sandbox project', async ({ githubAPI, projectsAPI, sandbox, dataManager }, issueId, statusName) => {
  const ts = Date.now();
  const rand = Math.random().toString(36).slice(2, 6);
  const title = `${issueId}-e2e-${ts}-${rand}`;
  ...
  const issue = await githubAPI.createIssue(env.github.testRepo, { title, labels: [], body: `Filter test issue ${issueId}` });
  const itemId = await projectsAPI.addIssueToProject(sandbox.projectId, issue.node_id);
